...
Status game_add_new_savefile(Game *game, char *name)
{
    ...
    sprintf(str, ">game_saves/%s.txt", name);
    system(str);
    /*Adds the savefile*/
    ...
    fprintf(data, "#f:game_saves/%s.txt\n", name);
...
Status game_load_savefile(Game *game, char *name)
{
    ...
    while (fgets(str, WORD_SIZE, file))
    {
        ...
        else if (strncmp("#o:", str, IDENTIFIER_LENGHT) == 0)
        {
            ...
            toks = strtok(NULL, "|");
            aux1 = atoi(toks);
            object_set_open((Object *)aux_structure, aux1);
        }
        ...
        else if (strncmp("#l:", str, IDENTIFIER_LENGHT) == 0)
        {
            ...
            toks = strtok(NULL, "|");
            aux1 = atoi(toks);
            link_set_state((Link *)aux_structure, aux1);
        }
        else
        {
...
Status game_save_game(Game *game, char *name)
{
    ...
    Id *ids = NULL;
    int i, j, aux1;
    char str1[2 * WORD_SIZE], str2[WORD_SIZE];
    /*Error management*/
    if (!(game) || !(name))
        return ERROR;
    ...
    {
        /*Gets the link id*/
        aux1 = link_get_id(game->links[i]);
        sprintf(str1, "#l:%d", aux1);
        strcpy(str2, str1);
        /*Gets the state of the link*/
        aux1 = link_get_state(game->links[i]);
        sprintf(str1, "%s|%d\n", str2, aux1);
        /*Prints the line in the file*/
        fprintf(file, "%s", str1);
    }
...
Status game_create_from_file(Game **game, char *filename)
{
    ...
    /*Saves the name of the datafile*/
    if (game_set_datafile(*game, filename) == ERROR)
    {
        game_destroy(game);
        return ERROR;
    }
    ...
    /*Loads the name of all the savefiles.*/
    if (game_reader_load_savefiles(*game, filename) == ERROR)
    {
        game_destroy(game);
        return ERROR;
    }
...
{
    /*Error management.*/
    if (!game || position < 0 || position >= game->n_savefiles)
        return NULL;

    /*Returns the name of the savefile.*/
    return game->savefiles[position];
}
...
Status game_find_savefile_by_name(Game *game, char *name)
{
    ...
    /*Searches for the savefile by its name.*/
    for (i = 0; i < game->n_savefiles; i++)
    {
        if (strcmp(game->savefiles[i], name) == 0)
            return OK;
    }
    /*The savefile wasn't found.*/
    return ERROR;
}
